...
if (count($_brands_) > 0 && $_brands_[0] != '') {
    ...
    foreach ($_brands_ as $brand) {
        $products = array_merge($products, Product::getAllByBrand($pdo, Product::getAllByGender($pdo, $MEN), $brand));
    }
} else {
    $products = Product::getAllByGender($pdo, $MEN);
}
...
$categories = Category::getAllByCategoryName($pdo, "Men");
$title = "Men products";
